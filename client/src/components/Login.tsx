...
const Login = ()=> {
    ...
    const handleSubmit = async (e:React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                // Set user in global context and navigate home
                setUser(data.user)
                navigate("/")
            }
        } catch (err) {
   ...
   <div className="min-h-screen flex items-center justify-center"> <form
                ...
                <button type="submit" disabled={loading} className="mt-2 w-full h-11 rounded-full text-white bg-pink-600 hover:bg-pink-500 disabled:opacity-60 disabled:cursor-not-allowed transition" >
